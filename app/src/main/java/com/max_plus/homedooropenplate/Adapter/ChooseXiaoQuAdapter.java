...
public class ChooseXiaoQuAdapter extends BaseAdapter {

    private Context mContext;
    private List<XiaoQuBean> mDatas = new ArrayList();

    public ChooseXiaoQuAdapter(Context context) {
        mContext = context;
    }
    ...
    @Override
    public XiaoQuBean getItem(int position) {
...
//        mDatas.get(position).setCheckStatus(mDatas.get(position).getCheckStatus()?true:false);
        ...
        mViewHolder.tv_title.setText(mDatas.get(position).getXiaoqu_Name());
        return convertView;
    }
