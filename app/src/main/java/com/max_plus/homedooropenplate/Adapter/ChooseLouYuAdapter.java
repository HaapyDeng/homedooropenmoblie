...
public class ChooseLouYuAdapter extends BaseAdapter {

    private Context mContext;
    private List<LouYuBean> mDatas = new ArrayList();

    public ChooseLouYuAdapter(Context context) {
        mContext = context;
    }
    ...
    @Override
    public LouYuBean getItem(int position) {
...
//        mDatas.get(position).setCheckStatus(mDatas.get(position).getCheckStatus()?true:false);
        ...
        mViewHolder.tv_title.setText(mDatas.get(position).getLouyu_Name());
        return convertView;
    }
